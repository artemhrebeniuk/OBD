...
В процессе эксплуатации приложения (особенно на Windows с Bluetooth ELM327) могут возникать следующие неполадки:

### 1. Программа не запускается (ошибки при старте)
* **Отсутствие библиотек**: Приложение падает с ошибкой `ImportError`. 
  * *Решение*: Убедитесь, что установлены все необходимые зависимости из `requirements.txt`: `pip install -r requirements.txt`.
* **Ошибка прав доступа (macOS / Linux)**: Ошибка `Permission denied` при попытке открыть устройство `/dev/tty.*`.
  * *Решение*: На Linux добавьте текущего пользователя в группу `dialout`: `sudo usermod -aG dialout $USER` и перезайдите в систему. На macOS убедитесь, что вы не используете устройство под учетной записью с ограниченными правами.

### 2. Программа запускается, но не подключается к авто (статус «Connection Failed»)
* **Занятость COM-порта (Port Lock)**:
  * *Описание*: Последовательный порт может быть занят только одним приложением в системе. Если запущена другая программа (например, OBD Auto Doctor, Car Scanner или терминал), наше приложение выдаст ошибку доступа (`Access is denied` или `Resource busy`).
  * *Решение*: Закройте все сторонние программы, работающие с адаптером.
* **Ошибка входящего/исходящего порта (Bluetooth на Windows)**:
  * *Описание*: Windows при сопряжении адаптера создает два виртуальных COM-порта: **Входящий (Incoming)** и **Исходящий (Outgoing)**. Наша программа сможет подключиться только к **Исходящему**.
  * *Решение*: Выберите нужный COM-порт вручную из выпадающего списка в шапке приложения. Узнать номер исходящего порта можно в *Параметрах Bluetooth -> Другие параметры -> COM-порты*.
* **Выключено зажигание автомобиля**:
  * *Описание*: Адаптер ELM327 получает питание от OBD-разъема напрямую, поэтому он моргает и отвечает на запросы при выключенном зажигании. Но сам бортовой компьютер автомобиля (ЭБУ) при этом спит.
  * *Решение*: Переведите ключ зажигания автомобиля в положение **ON** (или заведите двигатель).
* **Несовместимая версия адаптера (Дешевый клон v2.1)**:
  * *Описание*: Множество дешевых адаптеров с версией v2.1 не поддерживают стандартные команды инициализации протокола. Библиотека `python-obd` очень чувствительна к совместимости команд.
  * *Решение*: Используйте адаптер с честной версией прошивки **v1.5** (желательно на оригинальном чипе PIC18F25K80) либо включите тумблер **Safe Mode: ON** в приложении.
* **Зависание при автоматическом поиске (Auto Connect)**:
  * *Описание*: В автоматическом режиме программа последовательно опрашивает все COM-порты на трех скоростях (`38400`, `9600`, `115200`). Если в системе много портов, опрос может занять до 30 секунд.
  * *Решение*: Выберите порт адаптера вручную в выпадающем списке, чтобы подключение происходило мгновенно.
